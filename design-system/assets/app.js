...
const R = {
  ...
  'wallet-head': () => `<div class="r-scope" style="padding:20px;border-radius:16px;background:var(--wt-bg-chat)">${walletHead()}</div>`,
  ...
  'swap': () => `<div class="r-scope" style="padding:18px;border-radius:16px;background:var(--wt-bg-panel);box-shadow:var(--wt-shadow-1);display:grid;justify-items:center">${swapPanel()}</div>`,
  ...
  'overlays': () => `<div class="r-scope d-overlays"><div class="r-pop">${[['pin', '置顶'], ['mute', '消息免打扰'], ['burn', '阅后即焚']].map(([i, t], k) => `<div class="r-pop__item${k === 0 ? ' is-hover' : ''}">${ic(i)}${t}</div>`).join('')}<div class="r-pop__sep"></div><div class="r-pop__item is-danger">${ic('trash')}删除会话</div></div>
    ...
    <div style="display:grid;gap:12px;align-content:start"><div class="r-notify">${av('P', 1, { group: 1, size: 'md' })}<div style="min-width:0"><b>PEPE 早鸟群<small>刚刚</small></b><p>老王：今晚 22:00 开直播聊新合约</p></div></div><span class="r-toast is-ok">${ic('check')}地址已复制</span><span class="r-toast is-err">${ic('alert')}余额不足</span><span class="r-tip">聊天记录</span></div></div>`,
